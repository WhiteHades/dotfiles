...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float lineLength = length(delta);
     ...
     if (lineLength > minDist) {
        ...
        float head_delay_factor = (TRAIL_SIZE) / lineLength;

        float isLongMove = step(TRAIL_SIZE, lineLength);

        float head_eased_short = ease(progress);
        float tail_eased_short = ease(smoothstep(head_delay_factor, 1.0, progress));
        ...
        vec2 v0 = vec2(head_pos_tl.x + currentCursor.z * isTopRightLeading, head_pos_tl.y - currentCursor.w * TRAIL_THICKNESS);
        vec2 v1 = vec2(head_pos_tl.x + currentCursor.z * isBottomLeftLeading, head_pos_tl.y - currentCursor.w * (1.0 - TRAIL_THICKNESS));
        
        // v2, v3: "back" of the trail (tail)
        vec2 v2 = vec2(tail_pos_tl.x + currentCursor.z * isBottomLeftLeading, tail_pos_tl.y - currentCursor.w * (1.0 - TRAIL_THICKNESS));
        vec2 v3 = vec2(tail_pos_tl.x + currentCursor.z * isTopRightLeading, tail_pos_tl.y - previousCursor.w * TRAIL_THICKNESS);
        ...
        vec2 box_size = (max_center - min_center) + currentCursor.zw * vec2(TRAIL_THICKNESS_X, TRAIL_THICKNESS);
        ...
        // fade
        float trail_progress = clamp(distance(vu, head_center) / distance(head_center, tail_center), 0.0, 1.0);
        float fade = mix(1.0, 0.0, trail_progress * FADE_ENABLED);
        trail.a *= fade;
